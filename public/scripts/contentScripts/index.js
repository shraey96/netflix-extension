    ...
    Array.from(videoCardDOM).forEach((x) => {
      ...
      if (typeof moddedVideoState !== undefined) {
        ...
        if (moddedVideoState === 1) {
          // data.closest(".slider-item").classList.remove("extension-card-hide")
          // data.closest(".slider-item").classList.remove("extension-card-show")
      ...
      if (videoStateControllerDOM === null) {
        ...
        if (videoStateControllerDOM) {
          videoStateControllerDOM.style.display = isDesignMode ? "flex" : "none"
        }
      }
    })
...
const getShowToggleStatus = (videoId) => {
  ...
  if (isDesignMode) {
    ...
    if (
      ...
      matchScoreFilter &&
      videoIdRatings[videoIdRatings] < matchScoreFilter
    ) {
      // dim
      status = 1
    }
